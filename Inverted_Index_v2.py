...
USE_PARALLEL = False          # True = build index with multiple processes

stemmer = PorterStemmer()

# Global variable used only inside worker processes to hold stopwords in memory
...
def clean_text(text, stopwords):
    ...
    for w in words:
        ...
        w = re.sub(r"[^a-z0-9]", "", w)  # keep only letters/digits and drops punctuation/symbols
...
def _init_worker(stopwords):
    # Initializer function: runs once per process when the pool starts up,
    # avoiding the need to send the large stopwords list with every single task.
    global _worker_stopwords
    _worker_stopwords = stopwords


def process_single_document(args):
    # Worker wrapper function that uses the pre-initialized global stopwords
    doc_name, text = args
    return count_words(doc_name, text, _worker_stopwords)


def build_index(docs, stopwords):
    # normal, one after the other version: goes through every doc, counts its
...
def build_index_parallel(docs, stopwords):
    # Optimized parallel version: passes only doc names and text to workers,
    # utilizing an initializer to share stopwords efficiently.
    args_list = [(name, text) for name, text in docs.items()]
    
    # Cap processes to avoid over-allocating if there are fewer docs than cores
    num_procs = min(cpu_count(), max(1, len(docs)))

    index = {}
    with Pool(processes=num_procs, initializer=_init_worker, initargs=(stopwords,)) as pool:
        for doc_name, counts in pool.imap_unordered(process_single_document, args_list, chunksize=5):
            for w, c in counts.items():
                if w not in index:
                    index[w] = {}
                index[w][doc_name] = c
...
def save_index(index, path):
    # saves the index to disk as JSON so we don't have to rebuild it every time
...
def main():
    # the nltk needs these small models downloaded once to tokenize the text
    ...
    start = time.time()
    if USE_PARALLEL:
        print(f"\nBuilding inverted index in PARALLEL ({min(cpu_count(), max(1, len(docs)))} processes) ...")
